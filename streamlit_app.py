    ...
    try:
        ...
        for model_path in model_paths:
            if os.path.exists(model_path):
                ...
                st.success(f"✅ Loaded model: {model_path} ({contrast_status})")
                st.info(f"📊 Expected input shapes: {expected_shapes}")
...
def extract_features_from_audio(audio, target_time_dim=259):
    ...
    try:
        ...
        for n_mels in CONFIG["mel_bands"]:
            ...
            # Pad or crop mel_db to target_time_dim FIRST
            ...
            if CONFIG.get("use_spectral_contrast", False):
                ...
                # Combine mel spectrogram with spectral contrast
...
with st.sidebar:
    ...
    with st.container():
        ...
        st.info(f"Model: {len(CONFIG['instruments'])}-Class CNN")
...
with tab2:
    ...
    if "predictions" in st.session_state:
        ...
        threshold = st.slider(
            ...
            step=0.05,
            help="Adjusting this filter recalibrates the active instrument list",
        )
            ...
            with cols[idx % 3]:
                ...
                <div class="instrument-card {card_class}">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                        <h3 style="margin: 0; font-size: 1.1rem; color: #e2e8f0;">{inst}</h3>
...
"""
            for idx, inst in enumerate(CONFIG["instruments"]):
                if predictions[idx] > CONFIG["detection_threshold"]:
                    ...
                    status = "[SILENT]"
                text_report += (
                    f"\n{inst:20s} : {status:12s} | {predictions[idx]:.2%} Conf."
                )
